...
#### 6.1.1 Neural DQS Benchmark

**Dataset**: COCO128 controlled degradation variants
- Base: COCO128 (128 images, 80 classes)
- Variants: 96 total across 10 degradation categories
- Training: YOLOv11n, **15 epochs** (CPU, Intel Core Ultra 9 285H, 16 cores)
...
### 6.4 Ablation Study — DQS Feature Ablation（Table 2）

Leave-one-out feature ablation on the 96-variant benchmark (CV Pearson r, k=5):
...
SHAP mean |φ| (impact on predicted mAP):

| Feature | Mean \|φ\| |
|---------|-----------|
| CD — CLIP Diversity | **0.0765** |
| IQ — Image Quality  | 0.0211 |
| AQ — Annotation Q.  | 0.0142 |
| LD — Lighting Div.  | 0.0084 |
| PD — Pose Div.      | 0.0052 |
| CB — Class Balance  | 0.0050 |

SHAP confirms the ablation finding: CD dominates (3.6× larger than IQ). AQ ranks third despite its near-zero Pearson r — indicating it captures non-linear quality signal (label completeness penalizes missing annotations in a non-linear regime).
